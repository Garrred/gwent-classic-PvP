...
class Player {
	constructor(playerId, name, deck) {
		this.playerId = playerId;
		this.playerTag = (playerId === 0) ? "player1" : "player2";
		this.controller = (playerId === 0) ? new Controller() : null;
		
		this.hand = (playerId === 0) ? new Hand(document.getElementById("hand-row")) : new HandPlayer2();
		...
		this.name = name;
		document.getElementById("name-" + this.playerTag).innerHTML = name;
...
async function translateTo(card, container_source, container_dest){
	...
	function getSourceElem(card, source, dest){
		if (source instanceof HandPlayer2)
	...
	function getDestinationElem(card, source, dest){
		if (dest instanceof HandPlayer2)
