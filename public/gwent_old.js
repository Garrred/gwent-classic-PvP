    ...
    if (this.id === 0 && this.leader.activated.length > 0) {
      this.elem_leader.addEventListener(
        ...
        async () =>
          await ui.viewCard(
            this.leader,
            async () => await this.activateLeader()
          ),
        false
      );
    } else {
...
class Board {
  ...
  translateToOpponentRow(row) {
    console.log(row);
    console.log(this.row[5 - row.index]);
    return row === null ? null : this.row[5 - row.index];
  }
...
class Game {
  ...
  async initialRedraw() {
    ...
    await ui.queueCarousel(
      player1.hand,
      2,
      async (c, i) => await player1.deck.swap(c, c.removeCard(i)),
      (c) => true,
      true,
      true,
  ...
  async endRound() {
    let dif = player1.total - player2.total;
    if (dif === 0) {
      let nilf_me = player1.deck.faction === "nilfgaard",
        nilf_op = player2.deck.faction === "nilfgaard";
      dif = nilf_me ^ nilf_op ? (nilf_me ? 1 : -1) : 0;
    }
    let winner = dif > 0 ? player1 : dif < 0 ? player2 : null;
    let verdict = {
      winner: winner,
      score_me: player1.total,
      score_op: player2.total,
    };
    ...
    if (player1.health === 0 || player2.health === 0) this.endGame();
...
class UI {
  constructor() {
    ...
    document
      ...
      .addEventListener("click", () => player1.passRound(), false);
    document
  ...
  async selectRow(row) {
    ...
    // console.log(row);
    row = board.translateToOpponentRow(row);
    // console.log("hardcoded");
    // console.log(document.getElementById("opponent-rangedopponent-ranged"));
    this.lastRow = row;
